  ...
  shuffle() {
    ...
    while (currentIndex != 0) {
      ...
      [this.deckPile[currentIndex], this.deckPile[randomIndex]] = [
        this.deckPile[randomIndex],
        this.deckPile[currentIndex],
      ];
    }
  }
...
class Player {
  ...
  isValidCard(card, discardPile) {
    if (
      this.hand[card].cardColour ===
        discardPile[discardPile.length - 1].cardColour ||
      this.hand[card].cardNumber ===
        discardPile[discardPile.length - 1].cardNumber
    ) {
      this.validTurn = true;
      return true;
    } else {
